...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        rest = expressions.rest.map(lambda operand: scheme_eval(operand, env))
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        signature, formals = signature.first, signature.rest
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return is_scheme_true(expressions)
...
def make_let_frame(bindings, env):
    ...
    def check_bingings_name(name):
        if name in nameChecker:
            raise SchemeError("duplicate binding name")
        if not scheme_symbolp(name):
            raise SchemeError("non-symbol: {0}".format(name))
        nameChecker.append(name)

    def get_bingings_name_helper(bingings):
        cause = bingings.first
        validate_form(cause, 2, 2)
        if bingings.rest is nil:
            check_bingings_name(cause.first)
            return Pair(cause.first, nil)
        check_bingings_name(cause.first)
        return Pair(cause.first, get_bingings_name_helper(bingings.rest))

    def get_bingings_vals_helper(bingings):
        if bingings.rest is nil:
            return Pair(scheme_eval(bingings.first.rest.first, env), nil)
        return Pair(
            scheme_eval(bingings.first.rest.first, env),
            get_bingings_vals_helper(bingings.rest),
        )

    names = get_bingings_name_helper(bindings)
    vals = get_bingings_vals_helper(bindings)
    # END PROBLEM 14
