def shuffle(s):
    ...
    new_list = []
    lenOfS = len(s)
    for i in range(lenOfS // 2):
        new_list.append(s[i])
        new_list.append(s[i + lenOfS // 2])
    return new_list
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        flag1 = balanced(end(left(m)))
        flag2 = balanced(end(right(m)))
        m1 = total_mass(end(left(m)))
        m2 = total_mass(end(right(m)))
        if flag1 and flag2 and m1 * length(left(m)) == m2 * length(right(m)):
            return True
        else:
            return False
